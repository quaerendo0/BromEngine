...
class Renderer {
public:
  Renderer(const LogicalDevice &device, const Surface &surface,
...
private:
  ...
  std::vector<VkSemaphore> imageAvailableSemaphores{
      MAX_FRAMES_IN_FLIGHT}; // VkSemaphore - only GPU waits for GPU, CPU is not
                             // locked
  std::vector<VkSemaphore> renderFinishedSemaphores{MAX_FRAMES_IN_FLIGHT};
  std::vector<VkFence> inFlightFences{
      MAX_FRAMES_IN_FLIGHT}; // fence - makes CPU wait for GPU
