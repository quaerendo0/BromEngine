...
Vulkan::AbstractBuffer::AbstractBuffer(const LogicalDevice &device,
  ...
  if (vkCreateBuffer(device.getDevicePtr(), &bufferInfo, nullptr,
                     &bufferHandle) != VK_SUCCESS) {
    throw std::runtime_error("failed to create vertex buffer!");
  }

  VkMemoryRequirements memRequirements;
  vkGetBufferMemoryRequirements(device.getDevicePtr(), bufferHandle,
                                &memRequirements);
  ...
  allocInfo.memoryTypeIndex =
      findMemoryType(device.getParentPhysicalDevice().getPhysicalDevicePtr(),
                     memRequirements.memoryTypeBits, properties);

  if (vkAllocateMemory(device.getDevicePtr(), &allocInfo, nullptr,
