...
bool checkDeviceExtensionSupport(VkPhysicalDevice device) {
  ...
  std::set<std::string> requiredExtensions(
      PhysicalDevice::deviceExtensions.begin(),
...
SwapChainSupportDetails getSwapChainSupportImpl(VkPhysicalDevice physicalDevice,
...
bool isDeviceSuitable(VkPhysicalDevice device, VkSurfaceKHR surface) {
  ...
  if (extensionsSupported) {
    SwapChainSupportDetails swapChainSupport =
        getSwapChainSupportImpl(device, surface);
...
/*
 * Creates physical device by using instance (main Vulkan entity) and window
 * surface (VkSurfaceKHR), that was already created. If cannot create, throws
 * exception.
 */
PhysicalDevice::PhysicalDevice(const VkInstance &inst,
                               const VkSurfaceKHR &surface,
                               const Log::ILogger &logger)
    : referenceInstance{inst}, referenceSurface{surface}, logger{logger} {
  ...
  for (auto &device : devices) {
    ...
    if (isDeviceSuitable(device, surface)) {
      ...
      logger.logInfo("Device \"" + std::string(deviceProps.deviceName) +
                     "\" is suitable and initialized.");
    }
  }
...
PhysicalDeviceQueueFamilyIndexInfo
PhysicalDevice::getDeviceQueueFamiliesInfo() const {
